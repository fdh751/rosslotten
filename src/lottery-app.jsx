...
const STYLE = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-tag {
    ...
    color: var(--accent); background: var(--accent-light); border: 1px solid var(--accent-mid);
  ...
  .btn-danger {
    ...
    padding: 0 12px; height: 28px; font-size: 0.75rem; border-radius: 6px;
  }
  .btn-danger:hover { border-color: var(--danger); color: var(--danger); }
  .btn-publish {
    background: var(--success); color: #fff; padding: 0 22px; height: 44px;
    font-size: 0.9rem; font-weight: 600; border-radius: var(--radius-sm);
    border: none; cursor: pointer; font-family: 'DM Sans', sans-serif;
    display: inline-flex; align-items: center; gap: 7px;
    box-shadow: 0 1px 3px rgba(22,163,74,0.2), 0 4px 12px rgba(22,163,74,0.12);
    transition: all 0.15s;
  }
  .btn-publish:hover:not(:disabled) { background: #15803d; transform: translateY(-1px); box-shadow: 0 2px 6px rgba(22,163,74,0.3), 0 8px 20px rgba(22,163,74,0.18); }
  ...
  /* STATS BAR */
  .stats-bar {
    display: flex; margin-bottom: 24px; border: 1.5px solid var(--border);
  ...
  .batch-card {
    ...
    padding: 20px; animation: fadeUp 0.25s ease both; transition: border-color 0.15s;
  }
  ...
  .batch-letter-badge {
    font-family: 'DM Mono', monospace; font-size: 1.3rem; font-weight: 500; color: var(--text);
    background: var(--surface-2); border-radius: 7px; width: 40px; height: 40px;
  ...
  .input-text-sm {
    ...
    font-size: 0.8rem; padding: 7px 10px; outline: none;
  ...
  .ticket:hover::after {
    content: attr(data-n); position: absolute; bottom: calc(100% + 4px); left: 50%; transform: translateX(-50%);
    background: var(--text); color: #fff; font-size: 0.62rem; font-family: 'DM Mono', monospace;
    padding: 2px 5px; border-radius: 4px; white-space: nowrap; pointer-events: none; z-index: 10;
  }
  ...
  .input-prize {
    flex: 1; height: 38px; background: var(--surface); border: 1.5px solid var(--border);
    border-radius: var(--radius-sm); color: var(--text); font-family: 'DM Sans', sans-serif;
    font-size: 0.88rem; padding: 0 12px; outline: none; transition: border-color 0.15s, box-shadow 0.15s;
  }
  .input-prize:focus { border-color: var(--accent); box-shadow: 0 0 0 3px var(--accent-light); }
  .input-prize::placeholder { color: var(--text-3); }
  .btn-remove-prize {
    background: transparent; border: 1.5px solid var(--border); color: var(--text-3);
  ...
  .btn-add-prize {
    background: transparent; border: 1.5px dashed var(--border); color: var(--text-3);
    padding: 0 14px; height: 36px; font-size: 0.8rem; font-family: 'DM Sans', sans-serif;
    font-weight: 500; border-radius: var(--radius-sm); cursor: pointer;
    display: inline-flex; align-items: center; gap: 5px; transition: all 0.15s; margin-left: 64px;
  }
  ...
  .input-count {
    width: 88px; height: 44px; background: var(--surface); border: 1.5px solid var(--border);
    border-radius: var(--radius-sm); color: var(--text); font-family: 'DM Mono', monospace;
  ...
  .btn-draw {
    background: var(--accent); color: #fff; padding: 0 28px; height: 44px;
    font-size: 0.92rem; font-weight: 600; border-radius: var(--radius-sm);
    box-shadow: 0 1px 3px rgba(0,87,255,0.2), 0 4px 12px rgba(0,87,255,0.15);
    transition: all 0.15s; border: none; cursor: pointer; font-family: 'DM Sans', sans-serif;
    display: inline-flex; align-items: center; gap: 6px;
  }
  ...
  /* PUBLISH BANNER */
  .publish-banner {
    background: var(--success-light); border: 1.5px solid var(--success-mid);
    border-radius: var(--radius-sm); padding: 14px 18px;
    display: flex; align-items: center; justify-content: space-between; gap: 12px;
    flex-wrap: wrap; margin-top: 16px; animation: fadeUp 0.25s ease;
  }
  ...
  .public-header .drawn-at { font-size: 0.75rem; color: var(--text-3); margin-top: 6px; font-family: 'DM Mono', monospace; }
  ...
  .public-winner-row {
    ...
    border-bottom: 1.5px solid var(--border); animation: fadeUp 0.3s ease both;
  }
  ...
  .checker-result {
    ...
    font-size: 0.9rem; font-weight: 500; animation: fadeUp 0.2s ease;
  }
...
function getSold(unsold) {
  const sold = [];
  for (let i = 1; i <= 100; i++) if (!unsold.has(i)) sold.push(i);
  return sold;
}

function drawWinners(batches, count) {
  const batchPools = batches.map(b => ({
    letter: b.letter,
    sold: getSold(parseUnsold(b.unsoldRaw)),
  ...
  while (winners.length < count && attempts < count * 20) {
    ...
    const src = available.length > 0 ? pool
...
async function clearPublished() {
  try { await window.storage.delete(STORAGE_KEY, true); return true; }
  catch { return false; }
}
...
// ─── Nav ──────────────────────────────────────────────────────────────────────
function TopNav({ page, hasPublished }) {
  return (
    <nav className="topnav">
      <span className="topnav-brand">Winner Draw</span>
      <div className="topnav-links">
        <a className={`nav-link${page === 'admin' ? ' active' : ''}`} href="#admin">
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="1" y="1" width="5" height="5" rx="1.5" stroke="currentColor" strokeWidth="1.5"/><rect x="7" y="1" width="5" height="5" rx="1.5" stroke="currentColor" strokeWidth="1.5"/><rect x="1" y="7" width="5" height="5" rx="1.5" stroke="currentColor" strokeWidth="1.5"/><rect x="7" y="7" width="5" height="5" rx="1.5" stroke="currentColor" strokeWidth="1.5"/></svg>
          Admin
        </a>
        <a className={`nav-link${page === 'results' ? ' active' : ''}`} href="#results">
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M1 10L4.5 6.5L7 9L10.5 4.5L12 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          Results
...
function PublicPage() {
  ...
  const handleCheck = () => {
    ...
    setCheckResult(idx >= 0 ? { win: true, position: idx + 1, prize: data.winners[idx].prize } : { win: false });
  };
    ...
    <div className="public-app">
      ...
      <div className="public-winners">
        {data.winners.map((w, i) => (
          <div className="public-winner-row" key={i} style={{animationDelay: `${Math.min(i * 0.04, 0.6)}s`}}>
        ...
        <div className="checker-box">
          ...
          <div className="checker-row">
            <input
              ...
              maxLength={1}
              placeholder="A"
              onChange={e => { setCheckLetter(e.target.value); setCheckResult(null); }}
            ...
            <input
              ...
              min={1} max={100}
              placeholder="42"
              onChange={e => { setCheckNum(e.target.value); setCheckResult(null); }}
              onKeyDown={e => e.key === 'Enter' && handleCheck()}
            />
...
// ─── Admin Page ───────────────────────────────────────────────────────────────
function AdminPage({ publishedData, onPublish, onClearPublished }) {
  const [batches, setBatches] = useState([]);
  const [newLetter, setNewLetter] = useState('');
  const [newUnsold, setNewUnsold] = useState('');
  const [addError, setAddError] = useState('');
  const [drawCount, setDrawCount] = useState(10);
  const [prizes, setPrizes] = useState([{ label: '' }]);
  const [winners, setWinners] = useState(null);
  const [winnerSet, setWinnerSet] = useState(null);
  const [publishing, setPublishing] = useState(false);
  const [justPublished, setJustPublished] = useState(false);

  const totalSold = batches.reduce((s, b) => s + getSold(parseUnsold(b.unsoldRaw)).length, 0);
  ...
  const handleRemove = (letter) => { setBatches(prev => prev.filter(b => b.letter !== letter)); setWinners(null); setWinnerSet(null); };
  const handleUnsoldChange = (letter, val) => { setBatches(prev => prev.map(b => b.letter === letter ? { ...b, unsoldRaw: val } : b)); setWinners(null); setWinnerSet(null); };
  const addPrize = () => setPrizes(p => [...p, { label: '' }]);
  const removePrize = (i) => setPrizes(p => p.filter((_, j) => j !== i));
  const updatePrize = (i, val) => setPrizes(p => p.map((x, j) => j === i ? { label: val } : x));

  const handleDraw = () => {
    const result = drawWinners(batches, drawCount);
    const resultWithPrizes = result.map((w, i) => ({ ...w, prize: prizes[i]?.label?.trim() || null }));
    setWinners(resultWithPrizes);
    const map = new Map();
    result.forEach(w => { if (!map.has(w.letter)) map.set(w.letter, new Set()); map.get(w.letter).add(w.number); });
    setWinnerSet(map);
  ...
  const handlePublish = async () => {
    if (!winners) return;
    setPublishing(true);
    const payload = { winners, drawnAt: new Date().toISOString() };
  ...
  return (
    <div className="app">
      ...
      <div className="add-row">
        <input className="input-letter" value={newLetter} maxLength={1} placeholder="A"
          onChange={e => { setNewLetter(e.target.value); setAddError(''); }}
          onKeyDown={e => e.key === 'Enter' && handleAdd()} />
        <input className="input-text" value={newUnsold} placeholder="Unsold ticket numbers — e.g. 3, 7, 15–20, 45"
          onChange={e => setNewUnsold(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handleAdd()} />
        ...
        <>
          ...
          <div className="batches-grid">
            {batches.map(b => {
              ...
              const wSet = winnerSet?.get(b.letter) ?? new Set();
              return (
                <div className="batch-card" key={b.letter}>
                  ...
                  <input className="input-text-sm" value={b.unsoldRaw} placeholder="e.g. 3, 15–20, 45"
                    onChange={e => handleUnsoldChange(b.letter, e.target.value)} />
      ...
      <div className="section-label">Prizes <span style={{fontWeight:400, textTransform:'none', letterSpacing:0, color:'var(--text-3)', fontSize:'0.75rem'}}> — assign a prize description to the top winners (optional)</span></div>
      <div className="prizes-section">
        <div className="prize-rows">
          {prizes.map((p, i) => (
            <div className="prize-row" key={i}>
              <div className="prize-rank"><span>#{i + 1}</span></div>
              <input className="input-prize" value={p.label}
      ...
      <div className="draw-controls">
        <input type="number" className="input-count" value={drawCount} min={1} max={totalSold || 1}
        ...
        {winners && (
          <button className="btn btn-ghost" onClick={() => { setWinners(null); setWinnerSet(null); setJustPublished(false); }}>
            Clear results
          </button>
        )}
      </div>
            ...
            <div className="publish-banner">
              <div className="publish-banner-text">
                {justPublished ? (
                  ...
                  <>
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6.5" stroke="#16A34A" strokeWidth="1.5"/><path d="M8 5v3.5l2 2" stroke="#16A34A" strokeWidth="1.5" strokeLinecap="round"/></svg>
              ...
              <div style={{display:'flex', gap:8, alignItems:'center'}}>
                {justPublished && publishedData && (
                  <button className="btn btn-ghost" style={{height:36, fontSize:'0.8rem'}} onClick={handleClearPublished}>
                    Unpublish
                  </button>
                )}
      ...
      {winners && winners.length === 0 && (
        <p className="hint" style={{color: 'var(--danger)'}}>Not enough sold tickets to fulfil that draw count.</p>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const [publishedData, setPublishedData] = useState(undefined);

  useEffect(() => { loadPublished().then(setPublishedData); }, []);

  const page = hash === '#results' ? 'results' : 'admin';
